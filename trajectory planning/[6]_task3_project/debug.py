...
class Debugger(object):
    ...
    def draw_alltra(self,path_x, path_y):
        package = Debug_Msgs()
    ...
    def draw_all(self,path_x, path_y,nodelist):
        package = Debug_Msgs()
        #self.draw_roadmap(package, sample_x, sample_y, road_map)
        for i in range(len(nodelist)):
            if nodelist[i].parent is not None:
                self.draw_line(package,nodelist[i].x,nodelist[i].y,nodelist[i].parent.x,nodelist[i].parent.y)
        self.draw_finalpath(package, path_x, path_y)
        self.draw_points(package, path_x, path_y)
        self.sock.sendto(package.SerializeToString(), self.debug_address)
